...
class MomentMatchMarginalPosteriorNonMyopic(VariationalMutualInformationOptimizer):
    ...
    def differentiable_loss(self, *args, **kwargs):
        ...
        ############ each decision (need to fix logDetJgY) ############################
        self.hXY = 0
        self.hY = 0
        sign, logdetSx  = torch.linalg.slogdet(Sigma[:self.dim_lat,:self.dim_lat])
        if sign < 0:
        ...
        for i in range(self.dim_obs):
            sign, logdetS  = torch.linalg.slogdet(Sigma[:self.dim_lat+1+i,:self.dim_lat+1+i])
            if sign < 0:
                print("negative det")
            self.hXY = .5*logdetS +((self.dim_lat+1+i)/2)*(torch.log(2*self.pi_const*self.e_const))-torch.mean(logDetJfX)-torch.mean(logDetJgY)
            
            sign, logdetSy  = torch.linalg.slogdet(Sigma[self.dim_lat:self.dim_lat+1+i,self.dim_lat:self.dim_lat+1+i])
            if sign < 0:
                print("negative det")
            self.hY = .5*logdetSy +((1+i)/2)*(torch.log(2*self.pi_const*self.e_const))-torch.mean(logDetJgY)

        ######################### Shows Promise (Prev Gauss 1 run: 7.13) ##########################################
        # Loss = 0
        # for i in range(self.dim_obs):
        #     idx = torch.hstack((torch.arange(self.dim_lat,device=latents.device),(self.dim_lat+i)*torch.ones(1,dtype=torch.int,device=latents.device)))
        #     sign, logdetS  = torch.linalg.slogdet(Sigma[idx,:][:,idx])
        #     if sign < 0:
        #         print("negative det")
        #     hXYi = .5*logdetS +((self.dim_lat+1)/2)*(torch.log(2*self.pi_const*self.e_const))-torch.mean(logDetJfX)
        #     Loss += hXYi
        # Loss += -torch.mean(logDetJgY)

        ######################### Shows Promise (Prev Gauss 1 run: 6.83) ##########################################
        # sign, logdetS  = torch.linalg.slogdet(Sigma)
        # if sign < 0:
        #     print("negative det")
        ...
        #     if sign < 0:
        #         print("negative det")
        #     hXYi = .5*logdetS +((self.dim_lat+1+i)/2)*(torch.log(2*self.pi_const*self.e_const))-torch.mean(logDetJfX)
        #     Loss += hXYi
        # Loss += -torch.mean(logDetJgY)
        ...
        # MI = 0
        # for i in range(self.dim_obs):
        #     # sign, logdetSi  = torch.linalg.slogdet(Sigma[self.dim_lat:self.dim_lat+1+i,self.dim_lat:self.dim_lat+1+i])
        #     sign, logdetSi  = torch.linalg.slogdet(Sigma[self.dim_lat:self.dim_lat+1+i,self.dim_lat:self.dim_lat+1+i])
        #     if sign < 0:
        #         print("negative det")
        #     hYi = .5*logdetSi +((1+i)/2)*(torch.log(2*self.pi_const*self.e_const))
        #     MI += -hX-hYi
        # # MI += -torch.mean(logDetJfX)-torch.mean(logDetJgY)
...
def main_loop(
    ...
    for t in range(0, T):
        ...
        with torch.no_grad():
            ...
            ell = design[0][0].size()[0]-1#0
            # max_MI = -100
            # Sigmaxx = mi_loss_instance.Sigma[:p * num_sources,:p * num_sources].detach()
            # _, detx = torch.linalg.slogdet(Sigmaxx)
            # for i in range(design[0][0].size()[0]):
            #     sigmayi = mi_loss_instance.Sigma[p * num_sources+i:p * num_sources+i+1,p * num_sources+i:p * num_sources+i+1].detach()
            #     _,detyi = torch.linalg.slogdet(sigmayi)
            #     idx = torch.hstack((torch.arange(p * num_sources,device=device),(p * num_sources+i)*torch.ones(1,dtype=torch.int,device=device)))
            #     sigmaxyi = mi_loss_instance.Sigma[idx,:][:,idx].detach()
            #     _,detxyi = torch.linalg.slogdet(sigmaxyi)
            #     mi = (1/2)* (detx+detyi-detxyi)#(1/2)* (-detyi+detxyi)#
            #     if mi >= max_MI:
            #         Sigmayy = 1*sigmayi
            #         max_MI = 1*mi
            #         ell = 1*i
            ...
            design_init = torch.cat((design[0][0][:ell],design[0][0][ell+1:]))#0*design[0][0][1:]#
